...
import { PageTransition, StaggerContainer, StaggerItem, FadeInUp, ScaleIn } from '@/components/shared/MotionWrapper';
...
export default function DashboardPage() {
  ...
  return (
    <PageTransition>
      ...
      {/* Metrics Cards */}
      <StaggerContainer className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-6">
        <StaggerItem><MoneyCard title="Faturamento hoje" value={m.revenueToday} icon={DollarSign} trend={{ value: 12, label: 'vs ontem' }} /></StaggerItem>
        <StaggerItem><MoneyCard title="Faturamento mês" value={m.revenueMonth} icon={Receipt} trend={{ value: 8, label: 'vs mês anterior' }} /></StaggerItem>
        <StaggerItem><MoneyCard title="Clientes hoje" value={String(m.clientsToday)} icon={Users} /></StaggerItem>
        <StaggerItem><MoneyCard title="Próximos" value={String(m.upcomingAppointments)} icon={Calendar} /></StaggerItem>
        <StaggerItem><MoneyCard title="Taxa de faltas" value={`${m.noShowRate}%`} icon={TrendingDown} /></StaggerItem>
        <StaggerItem><MoneyCard title="Ticket médio" value={m.averageTicket} icon={Clock} /></StaggerItem>
      </StaggerContainer>
